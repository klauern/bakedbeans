...
public class BeanRegistry {
    ...
    public <T extends Configuration> Map<Class, T> mapBakedBeans()
            throws JsonParseException, JsonMappingException, IOException {
        this.configured_classes = reflections
        ...
        for (Class<?> configured_class : configured_classes) {
            ...
            if (parse_type.equals(ConfigType.YAML)) {
                configs.put(configured_class, (T) yaml_mapper.readValue(
                        Resources.getResource(cf.location()), configured_class));
            } else if (parse_type.equals(ConfigType.JSON)) {
                configs.put(configured_class, (T) json_mapper.readValue(
                        Resources.getResource(cf.location()), configured_class));
            } else if (parse_type.equals(ConfigType.XML)) {
                configs.put(configured_class, (T) xml_mapper.readValue(
                        Resources.getResource(cf.location()), configured_class));
            } else {
                throw new JsonMappingException("Unknown data format "
                        + parse_type);
            }
